...
important works by citation counts as cocitation; it is difficult to know whether
...
![PRISMA diagram describing construction of the coreDSEworks corpus](dse_citan_files/prisma.png)

##### Review data processing output
...
##### Review all coreDSEworks records

```{r}
#| label: display-core-works
## Review coreDSEworks records
...
showabit <- refWorks %>% head(100)
datatable(showabit)
```

###### Example Duplicated Record

```{r}
#| label: demo-dupliactes
showabit <- refWorks %>% filter( CR %like% "DATA SCIENCE FOR UNDERGRADUATES" )
datatable(showabit)
```

###### Cleaned Reference Lookup
...
datatable(cleanRefLookup %>% 
            filter(shortname %like% "biehler r 2018") %>% 
...
[Fig 3](#refnet) features a visualization of the reference co-citation network of all papers that are cited at by at least 3 distinct core DSE works. In both representations, three distinct clusters can be identified: a green cluster to the left of both \[XXX\]

```{r}
#| label: pretty-ref-net
# To regenerate the visualization shared here, use VOSviewer to
# visualize refNet as it is generated in the preceding code.
# Set attraction = 3; repulsion = 0
```

![Reference Network visualization generated by VOSviewer. Three distinct communities are evident in the network.](dse_citan_files/jul30refvos.png){#refnet}
...
refNet=networkPlot(refMatrix, n = cutoff,
                   ...
                   edges.min=0, type = "mds", cluster="louvain")#,
                   #vertex.color=)
...
To identify the most highly cited reference works in this network, I first identify all nodes with a betweenness centrality of over 150. This is quite high; the mean betweenness centrality of the network is \~80 and the median is \~7.5. I then sort these nodes in descending order by corrected citation frequency. The sorted list of all broker works is featured in Fig X.
...
#relevantRefs <- cleanRefLookup %>% 
#  filter(shortname %in% refBrokers$vertex) %>% 
#  filter(correctedFreq > 1) %>% 
#  arrange(desc(correctedFreq))

#datatable(relevantRefs)

#datatable(left_join(refBrokers, relevantRefs, by=c("vertex"="shortname") ) )
```
