...
![Figure 1. Three key data frames: coreDSEworks, refWorks that are cited in the coreDSEworks, and the bibNetwork that represents co-citation patterns of refWorks.](dse_citan_files/datasources.png){#data-sources}

### 2.1 Identifying "Core" Data Science Education Works
...
### 2.2. Extracting and Cleaning Reference Works
...
#### Community 1: A Focus on Data Science for Undergraduate Majors
...
coreCitingWorks1 <- coreDSEworks %>% 
  filter( str_count(coreDSEworks$refInfo,"1")
          /str_count(coreDSEworks$refInfo,"[1-3]") > .75 )

coreCitingWorks1 <- coreCitingWorks1 %>%
  filter( str_count(coreCitingWorks1$refInfo,"[1-3]") > 3 )

datatable(coreCitingWorks1[c(4,6,90,93,94)])

coreBibAnalysis <- biblioAnalysis(data.frame(coreCitingWorks1))
summary(coreBibAnalysis, max=10)
```

#### Community 2: A Focus on K-12 Education and the Learning Sciences
...
coreCitingWorks2 <- coreDSEworks %>% 
  filter( str_count(coreDSEworks$refInfo,"2")
          /str_count(coreDSEworks$refInfo,"[1-3]") > .75 )

coreCitingWorks2 <- coreCitingWorks2 %>%
  filter( str_count(coreCitingWorks2$refInfo,"[1-3]") > 3 )

datatable(coreCitingWorks2[c(4,6,90,93,94)])

coreBibAnalysis <- biblioAnalysis(data.frame(coreCitingWorks2))
summary(coreBibAnalysis, max=10)
```

#### Community 3: Data Science with a Computing Emphasis for Non-Majors
...
coreCitingWorks3 <- coreDSEworks %>% 
  filter( str_count(coreDSEworks$refInfo,"3")
          /str_count(coreDSEworks$refInfo,"[1-3]") > .75 )

coreCitingWorks3 <- coreCitingWorks3 %>%
  filter( str_count(coreCitingWorks3$refInfo,"[1-3]") > 3 )

datatable(coreCitingWorks3[c(4,6,90,93,94)])

coreBibAnalysis <- biblioAnalysis(data.frame(coreCitingWorks3))
summary(coreBibAnalysis, max=10)
```

### Exemplar Citing Works Per Community
