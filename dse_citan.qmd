...
datatable(coreCitingWorks1[c(4,6,90,94)])
...
datatable(coreCitingWorks2[c(4,6,90,94)])
...
datatable(coreCitingWorks3[c(4,6,90,94)])
...
bridges <- coreDSEworks %>% 
    filter( length(coreDSEworks$refInfo) > 5 )
  
#   then only include those that have >75% reWorks from cluster n
bridges1 <- bridges %>% 
  filter( sum(bridges$refInfo=="1") 
          /sum(bridges$refInfo=="[1-3]") > .1 )
  
bridges2 <- bridges1 %>% 
filter( sum(bridges1$refInfo=="2") 
        /sum(bridges1$refInfo=="[1-3]") > .1 )
    
bridges3 <- bridges2 %>% 
  filter( sum(bridges2$refInfo=="3") 
        /sum(bridges2$refInfo=="[1-3]") > .1 )

bridgeAnalysis <- biblioAnalysis(data.frame(bridges3))
summary(bridgeAnalysis, max=5)

datatable(bridges3[c(4,6,90,94)])
```
