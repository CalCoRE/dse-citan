...
coreDSEworks$raw_stripped_CR <- 
  as.list(str_split(gsub(charExcludeList,'',coreDSEworks$CR_raw),";")) #for debugging
...
refWorks <- correctFrequenciesCited(refWorks,coreDSEworks) # update frequencies given found matches in coreDSEworks
```

Now, we can check the NASEM report "Data Science Opportinities and Options" one more time and see that its various references formats have been consolidated, and this report appears near the top in terms of citation frequency across the coreDSEworks.

```{r}
#| label: peek-corrected-nasem2018
...
datatable(refWorks[c(1,4,7)] %>% 
            filter(refWorks$shortname %like% "sangsawang t 2006") %>%
...
refNet=networkPlot(refMatrix, n = 1,
...
datatable(left_join(refBrokers, relevantRefs, by=c("vertex"="shortname") )[c(1,3,8,9)] )
```
